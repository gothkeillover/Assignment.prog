...
void morseBlink(const char* pattern)
{
    for (int c = 0; pattern[c]!= '\0'; c++)
    {
        ...
        {
            segDP = 1; //DP ON
            thread_sleep_for(150); //blink
            segDP = 0; //DP OFF
        }
        else if (pattern[c] == '-')
        {
            segD = 1; //DP ON
            thread_sleep_for(400); //blink
            segD = 0; //DP OFF
        }
        thread_sleep_for(120); //pause before next blink
    }
    thread_sleep_for(250); //pause before next letter
}
...
int main() 
{
    ...
    while (1)
    {
        ...
        if (userButton == 0)
        {
            morseBlink("-.-."); //morse code for letter C
            showLetter(C);
            showLetter(O);
            showLetter(R);
            showLetter(I);
        ...
        if (extButton == 0)
        {
            morseBlink(".---"); //morse code for letter J
            showLetter(J);
            showLetter(E);
            showLetter(M);
            showLetter(A);
